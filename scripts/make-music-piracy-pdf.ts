 ...
 *
 * Minimal Microsoft Azure look:
 *   - Segoe UI typography throughout (regular, bold, light). No italics.
 *   - Almost no graphics: plain white pages, a single hairline rule under
 *     each heading, and a small line above the footer. No banners, chips,
 *     accent bars, callout boxes or decorative shapes.
 *   - A restrained Azure blue is used only for the top-level section
 *     title and the header eyebrow; body text is Fluent neutral ink.
 *
...
// ---------------------------------------------------------------- Colours
const AZURE_BLUE = "#0078D4"; // Communication Blue, used sparingly
...
/** Section heading. Level 1 = document title, level 2 = section, level 3 = sub. */
function heading(text: string, opts: { level?: 1 | 2 | 3; space?: number } = {}) {
  const { level = 2, space = 12 } = opts;
  const size = level === 1 ? 26 : level === 2 ? 16 : 11.5;
  ensureRoom(size + 20);
...
/** Small uppercase eyebrow above a title, in Azure blue. */
function eyebrow(text: string) {
  doc
    .font(F_BOLD)
    .fontSize(9.5)
    .fillColor(AZURE_BLUE)
    .text(text, { characterSpacing: 1.4, lineGap: 0 });
  doc.moveDown(0.35);
}
...
function paragraph(
  ...
  const { muted = false, size = 10.5 } = opts;
  ensureRoom(size + 8);
  doc
    .font(F_REGULAR)
    .fontSize(size)
...
function bulletList(items: string[]) {
  ...
  for (const item of items) {
    ...
    doc
      ...
      .fontSize(size)
      .fillColor(INK)
      .text(item, textX, y, { width: textWidth, align: "left", lineGap: 3.2 });
    doc.moveDown(0.45);
  }
  doc.moveDown(0.2);
}

/** A block of quoted text (used for the reader's question). Rendered as
 *  a slightly indented, secondary-coloured paragraph — no box, no italic. */
function quoteBlock(label: string, text: string) {
  ensureRoom(60);
  const width = PAGE_W - MARGIN_X * 2 - 14;
  doc
    .font(F_BOLD)
    .fontSize(10.5)
    .fillColor(AZURE_BLUE)
    .text(label, MARGIN_X + 14, doc.y, { width, lineGap: 0 });
  doc.moveDown(0.15);
  doc
    .font(F_REGULAR)
    .fontSize(10.5)
    .fillColor(INK_SECONDARY)
    .text(text, MARGIN_X + 14, doc.y, { width, align: "left", lineGap: 3.2 });
  doc.moveDown(0.6);
  doc.x = MARGIN_X;
}
...
function pageFooter(pageNum: number, total: number) {
  ...
  doc
    ...
    .fontSize(8.5)
    .fillColor(INK_SECONDARY)
    .text("Music Piracy", MARGIN_X, y, {
      width: (PAGE_W - MARGIN_X * 2) / 2,
      align: "left",
      lineBreak: false,
    });
  doc
    ...
    .fontSize(8.5)
    .fillColor(INK_SECONDARY)
    .text(`Page ${pageNum} of ${total}`, PAGE_W / 2, y, {
      width: (PAGE_W - MARGIN_X * 2) / 2,
      align: "right",
      lineBreak: false,
    });
...
doc.x = MARGIN_X;
doc.y = MARGIN_TOP + 40;
eyebrow("US 114055  \u00b7  NQF 5  \u00b7  ETHICS & PROFESSIONALISM");
doc.moveDown(0.3);

// Document title — Segoe UI Light, no ornament.
doc
  .font(F_LIGHT)
  .fontSize(48)
  .fillColor(INK)
  .text("Music Piracy", MARGIN_X, doc.y, { lineGap: 0 });
...
doc
  ...
  .text(
    ...
    MARGIN_X,
    doc.y,
    { width: PAGE_W - MARGIN_X * 2 - 30, lineGap: 4 },
  );

// Thin blue rule under the title block.
doc.moveDown(1.2);
doc
  .save()
...
heading("In this reader", { level: 3, space: 6 });
bulletList([
...
heading("Why this matters for US 114055", { level: 3, space: 8 });
paragraph(
...
heading("Answer", { level: 3, space: 4 });

paragraph(
  "Music piracy, while not as prominent as the old skull-and-crossbones kind, is certainly a good deal more widespread. \u2018Piracy\u2019 is generally considered to include:",
);
bulletList([
  "Pirate recordings \u2014 where it\u2019s just the music itself that is copied, usually by ordinary people using ordinary equipment on a not-for-profit basis (\u201CI just got the new album \u2014 do you want me to burn you a copy?\u201D).",
...
paragraph(
  "RIAA, the Recording Industry Association of America, claims that the recording industry loses around 4.2 billion U.S. dollars to piracy each year. This figure is reached by way of an inference that each pirate transaction represents a lost legitimate sale. This is obviously overly simplistic: people buy pirated music, or make their own copies, because doing so is cheaper than buying the real thing from a retailer. It is not at all obvious that if the pirated version were not available then all those people would head straight for the nearest music store. That said, it is pretty obvious that full-blown music counterfeiting is both illegal and unethical, and we ought not to support this industry by buying cheap counterfeits at flea markets and street traders. Your query, however, is about home-made pirate recordings.",
);
paragraph(
  "Many people who make pirate copies of CDs, particularly the \u201Chome pirates\u201D who don\u2019t actually make money out of piracy, think of themselves as modern day Robin Hoods \u2014 stealing from the obscenely rich recording company executives and their overpaid artists, and giving to the poor (themselves). RIAA tries to undermine that kind of thinking by claiming that it is the consumer who is the \u201Cultimate victim\u201D of piracy \u2014 because the consumer who buys a pirated copy is thereby denied the superior sound quality and proper packaging that comes with the real thing.",
);
paragraph(
  "While RIAA\u2019s argument is not particularly convincing, it is not clear that the Robin Hood argument holds water either. Presumably the idea is that in this sort of case stealing is justified by the wealth of the person being stolen from, and the real need of the person the stealing is supposed to benefit. But does this really hold in this case? While it is hard to feel too sorry for either a large label or their latest boy-band, we do need to ask ourselves whether they are really doing something wicked by making money out of their product. Perhaps the argument is that CDs are overpriced. Well, in a free-market system there is a pretty good way of driving some product\u2019s price down: don\u2019t buy it. If enough people agree with you, the product won\u2019t sell and the price will eventually come down. If it doesn\u2019t, then you were probably wrong about the overpricing in the first place. And music is not such a fundamental need that you might die while waiting for the market to make things right \u2014 if you are really desperate, there is always the radio.",
);
paragraph(
  "There are those who believe that there is price fixing going on in the music industry, and that piracy is a legitimate form of protest against the music barons. But if protest is really your goal, rather than just a convenient excuse for copying, then copying your friend\u2019s new CD is not a particularly effective way of doing it. Protests need to attract attention. If you really want to protest, and don\u2019t like any of the wide range of legal means of protesting available, then one route would be to openly sell counterfeit CDs outside your nearest big-name record store, and wait to get arrested. You would then get more than enough opportunity to get your message out through the media.",
);
paragraph(
  "If protest is not really what is on your agenda, what should you do about the situation you describe? If you really like the CD, buy yourself a legitimate copy and get rid of the pirated version. If your friend is concerned about why you didn\u2019t keep the pirated CD, you can always try RIAA\u2019s \u201Cultimate victim\u201D line. And if you decide not to buy the CD, then get rid of the pirated version anyway, and go out and buy two copies of the latest release from your favourite South African artist or band \u2014 one for you and one for your friend.",
);
...
paragraph(
  "The Business Software Alliance (BSA) \u2014 an anti-piracy umbrella body made up of large software companies \u2014 is getting tougher on offenders. Although in the past few months R300 000 has been recovered in out-of-court settlements, the BSA will in future prosecute offenders to the full extent of the law.",
);
paragraph(
  "The latest world-wide software piracy figures released by the BSA show that South Africa is one of the few countries in the world that suffered from an increased rate of software piracy from 1997 to 1998.",
);
paragraph(
...
paragraph(
  "As of 1 July 1999 the South African Government has six months to improve the local protection of intellectual property rights or face further unpleasantness from the Clinton administration\u2019s fair trade enforcers. The US trade representative will be seeking assurances that government computers have been purged of unlicensed software and that SA will be in full compliance with the World Trade Organisation\u2019s trade-related intellectual property agreement (TRIPS) by 1 January next year.",
);
paragraph(
  "The South African law does allow for some enforcement of copyright violation, and amendments to the copyright law in South Africa (the Intellectual Property Laws Amendment Act, effective 1 October 1997) have brought SA closer to compliance with its TRIPS obligations. This is especially true in the scope of protection given to computer programs, protection of compilations of data and databases, and terms of protection for audio-visual works. However, even after these amendments, numerous areas of South Africa\u2019s enforcement practices fall short of full TRIPS compliance.",
);
...
paragraph(
  "Although piracy rates among local businesses dropped 1% compared to last year, industry players said the numbers were still significantly high, representing at least R1,2 billion in economic losses. \u201CSoftware piracy remains one of the major hurdles to realising the potential of the information economy in South Africa, on the continent and around the world,\u201D said chairperson of the local arm of the Business Software Alliance (BSA), Stephan le Roux.",
);
paragraph(
  "Le Roux was speaking at the release of a study commissioned by BSA, which found that some countries had piracy rates topping 90%, with Africa and the Middle East\u2019s gross domestic product losing about R10,3 billion last year. BSA is an industry body representing commercial software developers and their hardware partners. Its vice-chairperson Andrew Lindstrom said plenty of jobs could be created if the intellectual property protection laws were robustly enforced.",
);
paragraph(
  "Besides software, South African intellectual property laws cover industries such as film, books and music.",
);
paragraph(
  "The BSA study comes amid concerted effort from law enforcement officials to crack down on DVD piracy, which crippled sales of the latest Leon Schuster movie Mama Jack and Oscar-winner Tsotsi. A group of music heavyweights recently raided Johannesburg streets to wipe out pirated music CDs sold in some shops and by street vendors. BSA said that software piracy in the local industry has led to unfair competition, which has jeopardised foreign direct investment from international companies as there were low market returns.",
);
paragraph(
  "Analysts said government needed to use all avenues to tackle the steep unemployment rate, which is sitting at 26,7%.",
);
paragraph(
  "The report covering 97 countries pointed to a global software piracy rate of 35%, unchanged from last year\u2019s level. \u201CThis represents at least $34 billion in economic losses worldwide \u2014 calculated according to the retail value of pirated software,\u201D BSA said.",
);
paragraph(
...
paragraph(
  "BSA said that if the global piracy rate were to drop 10% to 25%, about 2,4 million new jobs would be created, and a further $67 billion in tax revenues would be added worldwide.",
);
...
bulletList([
  "End User Copying \u2014 A licensed software user passes their software on to friends, business colleagues and family to copy indiscriminately. Or, in the case of volume software licenses, users and businesses under-report the number of computers on which the software is installed.",
...
paragraph(
  "Software piracy negatively impacts customers, resellers and software vendors. Lower vendor revenues, as a result of software piracy, limit the industry\u2019s ability to re-invest funds in research and development (R&D) and to continuously maintain and improve service and support infrastructures. Ongoing investment in R&D ensures that software vendors have the ability to keep their users at the forefront of the latest technology developments.",
);
paragraph(
